...
| `GEMINI_IMAGE_MODEL` | `imagen-4.0-generate-001` | Image/hero generation model ID |
...
| `GENERATE_HERO_IMAGE` | `true` | Whether to generate hero images via the image model |
...
`MAX_OPEN_BOT_PRS=1` is load-bearing: the daily cap limits PR *creation*, not publication. Without it, a merged backlog puts several back-dated posts live at once.
...
| `IMAGE_TIMEOUT_MS` | `180000` | 5,000–600,000 | Timeout for image generation calls |
